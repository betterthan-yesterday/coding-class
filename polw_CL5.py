...
def get_user_data(filename: str) -> list:
    ...
    for line in lines:
        user_password = line.strip("\n").split("\t") # cleans string and creates list of username and password
        # inputs username and password into appropriate list
        usernames.append(user_password[0])
        passwords.append(user_password[1])
        
    return [usernames, passwords] # returns combined list

#********************************************
# Write your exists function below: (2.5 marks)
#********************************************

def exists(username: str, username_list: list) -> bool:
    
    return username in username_list # returns if username exists in username_list

#********************************************
# Write your create_user function below: (2.5 marks)
#********************************************

def create_user(username: str, password: str, username_list: list, filename: str) -> bool:
    
    # checks if user is already in database
    if not exists(username, username_list):
        file = open(filename, "a")
        file.write(f"{username}\t{password}\n") # if username is not in database, write username and password
        file.close()
...
def login(username: str, password: str, filename: str) -> bool:
    
    # gets both the username list and password list
    username_list = get_user_data(filename)[0]
    password_list = get_user_data(filename)[1]

    if exists(username, username_list):
        # finds the index of username and checks password with the correct password in password_list
        return password == password_list[username_list.index(username)] 
...
# ```
# 1.
# If create_user() opened the file in write mode instead of append mode, the file would be cleared and all past data would be lost unless read first. If write mode was required to be used, the code should read the file and store the data before opening the file in write mode. Then write all stored data and also the new user.
# 
# 2.
# As passwords are required to be larger than 6 characters, valid_length() would be used in the create_user() function to ensure the length of the password passes the requirements. valid_length() would consist of len(string) > 6.
# 
# 3.
# encrypt() would be used in the create_user() function in order to encrypt the password that is about to written into the database. Likewise, decrypt() would be used in login() only when the password is about to be read.
# ```

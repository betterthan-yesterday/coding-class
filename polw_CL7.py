...
def selection_sort(nums: list) -> list: # sorts a list using selection sort
    
    sort = nums.copy()
    for i in range(len(sort)-1):
        min_index = sort.index(min(sort[i:]))
...
def extract_temps(temps: list[str]) -> list: # converts list of strings to list of floats
    ...
    return arr
    #return [float(i) for i in temps if i.replace('.','',1).isdigit()]

#********************************************
# Write your calculate_median function below: (4 marks)
#********************************************

def calculate_median(nums: list) -> float: # calculates median of list
...
def main(temp_data):
    ...
    try:
        return calculate_median(selection_sort(extract_temps(temp_data)))
    except:
        return "N/A"
print(main(["1","NA","5.5", True] ))
...
# 
# Boundary Case
# Function: calculate_median(temps)
# Input: temps = ["1","NA","5.5"]  
# Expected Output: [1.0,5.5]
# Output: [1.0,5.5]
# Pass/Fail: Pass
...
# ```
# 1. Any list that would return an empty list after being inputted into extract_temps would cause the function to return "N/A".
# 2. It is necessary because the try except may remove an error that would get called in the main function.
# ```

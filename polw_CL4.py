...
def update_required(device_info: list) -> bool:
    OSVersion = float(device_info[2])
    if OSVersion >= 8.0: return True

#********************************************
# Write your determine_update function below: (4 marks)
#********************************************

def determine_update(device_info: list) -> str:
    
    update_req = update_required(device_info) # a boolean representing if an update is required
    if not update_req: return "No Update"
        
    OSType = device_info[1]
    if OSType == "iOS": return "Version 1"

    OSVersion = float(device_info[2])
    if OSVersion >= 10.0: return "Version 2"

    month = int((device_info[3]).split('-')[1]) # Split the date by -, take the middle element, convert to int
    if month <= 6: return "Version 3"
    else: return "Version 4"

#********************************************
# Write your determine_device_update function below: (4 marks)
#********************************************

def determine_device_update(devices: list) -> list:
    
    device_update = []
    
    for device in devices:
        if update_required(device): device_update.append([device[0], determine_update(device)])
...
def main():
    devices = [['GX010365', 'iOS', '8.1', '2019-04-10'], #Version 1
              ...
              ['GX031153', 'Android', '9.0', '2019-07-11'], #Version 3
...
# ```
# 1. 
# The function update_required is not necessarily required, but I believe that it makes the code more readable because, without the function, every "update_required()" would have to be replaced with "float(device_info[2]) >= 8.0".
# 
# 2.
# A while loop can be used instead of a for loop if it is made to iterate through the list. We can iterate through the list with a while loop if we have an index counter that increments after every iteration of the while loop. However, this method uses extra memory, uses an extra function while serving no external purpose. A while loop would be more beneficial if the number of iterations of the loop is not specified beforehand.
# ```
